...
// The API key is retrieved from environment variables, which should be configured in the deployment environment.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });

export const generateDescription = async (prompt: string): Promise<string> => {
  // A check to ensure the API key is available at runtime.
